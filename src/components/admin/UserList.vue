    ...
    <div>
        ...
        <el-dialog title="修改用户信息" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
            <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="70px">
                <!-- 用户名 -->
                <el-form-item label="用户名" prop="username">
                    <el-input v-model="editForm.username" dieabled></el-input>
                </el-form-item>
                <!-- 密码 -->
                <el-form-item label="密码" prop="password">
    ...
    export default {
        ...
        methods: {
            ...
            async userStateChanged(userInfo) {
                const {
                    date: res
                } = await this.$http.put(`userstate?id=${userInfo.id}&state=${userInfo.state}`);
                if (res != "success") {
            ...
            async showEditDialog(id){
                ...
                this.editForm = res;//查询出的用户信息反填到编辑表单
                this.editDialogVisible = true;//开启編輯对话框
            },
            // 关闭窗口
            editDialogClosed(){
                this.$refs.editForm.resetFields();//重置信息
            },
            // 确认修改
            editUserInfo(){
